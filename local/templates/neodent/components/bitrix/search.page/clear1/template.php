...
<div class="search-page">
	...
	<form action="" method="get">
      ...
      <div class="input-group mb-3">
					...
					<?endif;?>
       <input class="input-group-text" type="submit" value="<?echo GetMessage("CT_BSP_GO")?>" />
      </div>
...
<?if(isset($arResult["REQUEST"]["ORIGINAL_QUERY"])):
	...
	<?elseif(count($arResult["SEARCH"])>0):?>
		...
		<?foreach($arResult["SEARCH"] as $arItem):?>
         ...
         <?
              $item = GetIBlockElement($arItem["ITEM_ID"]);
              $image_prw = CFile::GetPath($item["PREVIEW_PICTURE"]);
          ?>
      <?if($image_prw!=''):?>
      <a href="<?echo $arItem["URL"]?>" class="search-img">
        <img src="<?=$image_prw;?>" alt="<?echo $arItem["TITLE_FORMATED"]?>" />
      </a>
      <!-- /.search-img -->
      <?endif?>
